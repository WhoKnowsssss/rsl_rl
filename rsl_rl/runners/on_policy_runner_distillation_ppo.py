...
import rsl_rl
from rsl_rl.algorithms import PPO, Distillation, DistillationPPO
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        elif self.alg_cfg["class_name"] == "DistillationPPO":
            self.training_type = "distillation_ppo"
        else:
        ...
        if self.training_type == "rl":
            if "critic" in extras["observations"] or self.training_type == "distillation_ppo":
        ...
        if self.training_type == "rl" or self.training_type == "distillation":
            policy: ActorCritic | ActorCriticRecurrent | StudentTeacher | StudentTeacherRecurrent = policy_class(
                num_obs, num_privileged_obs, self.env.num_actions, **self.policy_cfg
            ).to(self.device)
        elif self.training_type == "distillation_ppo":
            num_teacher_obs = extras["observations"]["teacher"].shape[1]
            policy: ActorCriticTeacher = policy_class(
                num_obs, num_privileged_obs, num_teacher_obs, self.env.num_actions, **self.policy_cfg
            ).to(self.device)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.training_type == "distillation_ppo" and not self.alg.policy.loaded_teacher:
            raise ValueError("Teacher model parameters not loaded. Please load a teacher model to distill.")
        ...
        if self.training_type == "distillation_ppo":
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    # Sample actions
                    if self.training_type == "distillation_ppo":
                        actions = self.alg.act(obs, privileged_obs, teacher_obs)
                    else:
                    ...
                    if self.privileged_obs_type is not None:
                        ...
                        privileged_obs = obs
                    if self.training_type == "distillation_ppo":
                        teacher_obs = infos["observations"]["teacher"].to(self.device)
